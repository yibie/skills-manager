...
// skm — Effective-Agent 检视器 CLI(M1:validate / check)
//
...
/// 解析 `--flag value` 形式的选项,返回 (位置参数, 选项表)。
...
func validateSpec(specURL: URL, schemaURL: URL, quiet: Bool) -> AdapterSpec {
    let raw: JSONValue
    do {
