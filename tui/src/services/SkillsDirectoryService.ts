...
interface DirectorySkillPayload {
  source?: string
  skillId?: string
  name?: string
  installs?: number
...
function findBalancedSection(input: string, startIndex: number, opening: string, closing: string): string | null {
  let depth = 0
  for (let index = startIndex; index < input.length; index += 1) {
    ...
    if (character === opening) depth += 1
      ...
      depth -= 1
      if (depth === 0) return input.slice(startIndex, index + 1)
    }
  }
  return null
}

function extractEscapedJSONArray(html: string, key: string): string | null {
  const marker = `\\"${key}\\":`
...
function buildDiscoverEntries(payloads: DirectorySkillPayload[]): DiscoverSkill[] {
  const seen = new Set<string>()
  const entries: DiscoverSkill[] = []
...
function parseDirectoryViaPayload(html: string): { entries: DiscoverSkill[]; total: number } | null {
  const rawArray = extractEscapedJSONArray(html, 'initialSkills')
  if (!rawArray) return null

  try {
    const payloads = JSON.parse(rawArray.replace(/\\"/g, '"')) as DirectorySkillPayload[]
...
function parseDirectoryViaRegex(html: string): { entries: DiscoverSkill[]; total: number } {
  const patterns = [
    /\{\\"source\\":\\"([^\\]+)\\",\\"skillId\\":\\"([^\\]+)\\",\\"name\\":\\"([^\\]+)\\",\\"installs\\":(\d+)\}/g,
    /\{"source":"([^"]+)","skillId":"([^"]+)","name":"([^"]+)","installs":(\d+)\}/g,
  ]

  const payloads: DirectorySkillPayload[] = []
...
function parseDirectory(html: string): { entries: DiscoverSkill[]; total: number } {
  return parseDirectoryViaPayload(html) ?? parseDirectoryViaRegex(html)
}

function extractFirstParagraph(html: string | undefined): string | undefined {
...
export async function syncSkillsDirectory(): Promise<{ entries: DiscoverSkill[]; total: number }> {
...
export function loadSkillsDirectory(): { entries: DiscoverSkill[]; total: number } {
  const cached = readCache()
  if (cached) return { entries: cached.entries, total: cached.total }
  try {
...
export async function fetchDiscoverSkillDetail(entry: DiscoverSkill): Promise<DiscoverSkill> {
